...
if(b.action==='start'){
const config=configSchema.parse(b.config);if(data.setup)throw new HttpError(409,'시작할 때 정한 관찰 기준은 고정되어 있어요.');const saved=await db.prepare('INSERT OR IGNORE INTO observations (user_id,config,created_at) VALUES (?,?,?)').bind(user.id,JSON.stringify(config),now).run();if(!saved.meta.changes)throw new HttpError(409,'관찰 기준이 이미 저장됐어요. 저장된 기준을 다시 확인해 주세요.');
