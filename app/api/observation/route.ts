...
export async function observationData(userId:string){const db=database();const setup=await db.prepare('SELECT * FROM observations WHERE user_id=?').bind(userId).first<any>();const days=(await db.prepare('SELECT * FROM observation_days WHERE user_id=? ORDER BY created_at,id').bind(userId).all<any>()).results;const sum=days.reduce((a:number,b:any)=>a+b.value,0);return {setup:setup?{...setup,config:JSON.parse(setup.config),change:setup.change?JSON.parse(setup.change):null}:null,days,sum,average:days.length?Math.round(sum/days.length*100)/100:0,before:days.length>=2?(days[0].value+days[1].value)/2:null,after:days.length>2?Math.round(days.slice(2).reduce((a:number,b:any)=>a+b.value,0)/(days.length-2)*100)/100:null}}
...
if(b.action==='start'){
const config=configSchema.parse(b.config);if(data.setup)throw new HttpError(409,'시작할 때 정한 관찰 기준은 고정되어 있어요.');await db.prepare('INSERT INTO observations (user_id,config,created_at) VALUES (?,?,?)').bind(user.id,JSON.stringify(config),now).run();
...
await db.prepare('INSERT INTO observation_days (id,user_id,day,value,note,created_at) SELECT ?,?,?,?,?,? WHERE (SELECT count(*) FROM observation_days WHERE user_id=?)<5').bind(crypto.randomUUID(),user.id,today,value,note,now,user.id).run();
...
await db.prepare('UPDATE observations SET change=? WHERE user_id=? AND change IS NULL AND (SELECT count(*) FROM observation_days WHERE user_id=?)=2').bind(JSON.stringify(change),user.id,user.id).run();
